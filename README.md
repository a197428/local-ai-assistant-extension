...
Локальный AI-ассистент для Chrome, который открывается в боковой панели и помогает работать с информацией прямо из браузера. Он умеет читать контекст текущей страницы, искать данные в интернете, суммаризировать найденные материалы, объяснять сложные фрагменты, сравнивать источники и сохранять диалог в Markdown.

Проект собран как связка Chrome MV3 extension + Python backend. Расширение работает с UI на React, а backend на FastAPI запускает LangGraph-агента с OpenAI-compatible DeepSeek API и инструментами EXA для интернет-поиска и получения ответов по внешним источникам.
...
- Интернет-поиск через EXA, когда данных страницы недостаточно или пользователь просит найти новую информацию.
- Суммаризация найденных материалов, статей и результатов поиска.
- Объяснение, сравнение и структурирование информации из страницы и внешних источников.
- Поиск фактов, ссылок и кратких ответов без ручного переключения между вкладками.
...
- "Кратко перескажи эту страницу и выдели главное."
- "Найди в интернете свежую информацию по этой теме и сделай summary."
- "Сравни данные на странице с тем, что есть в открытых источниках."
- "Объясни этот текст простыми словами."
- "Собери список ссылок и фактов по теме."
- "Сделай конспект диалога и сохрани его в Markdown."
